...
function Element(props) {
  ...
  useEffect(() => {
    async function ApiData() {
      axios.get(`https://deploy-server-123.herokuapp.com/api/san-pham/${id}`)
        .then(resp => {
  ...
  useEffect(() => {
    async function fetchData() {
      axios
        .get("https://deploy-server-123.herokuapp.com/api/gio-hang", { withCredentials: true })
        .then((res) => {
          const { data } = res.data;
          setOrder(data);
        });
  ...
  const handleAddProduct = async (e) => {
    await axios
      .post("https://deploy-server-123.herokuapp.com/api/gio-hang", param, {
