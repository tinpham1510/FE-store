  ...
  useEffect(()=>{
    async function getData(){
      axios.get(`https://deploy-server-123.herokuapp.com/api/bao-cao/bao-cao-doanh-thu?tu_ngay=2021-07-01&den_ngay=2021-07-31`, { withCredentials: true })
                ...
                axios.get('https://deploy-server-123.herokuapp.com/api/don-hang/quan-ly', { withCredentials: true })
                .then(resp => {
                    var sum = resp.data.data.map(
                        (item, index) => setTongDonHang(index + 1)
                    )
                });
                axios.get('https://deploy-server-123.herokuapp.com/api/san-pham', { withCredentials: true })
                .then(resp => {
                    resp.data.data.map(
                        (item, index) => setTongSanPham(index + 1)
                    )
                });
                axios.get('https://deploy-server-123.herokuapp.com/api/san-pham?ma_loai_san_pham=6XQH7tKAXICRMeXkJzYy', {withCredentials: true}
                ).then(resp=>{
                  resp.data.data.map(
                    (item,index) => setLaptop(index +1)
                  )
                })
                axios.get('https://deploy-server-123.herokuapp.com/api/san-pham?ma_loai_san_pham=zgFsfEG1Dd5KP2S7ZD8h', {withCredentials: true}
                ).then(resp=>{
                  resp.data.data.map(
                    (item,index) => setPhone(index +1)
                  )
                })
                axios.get('https://deploy-server-123.herokuapp.com/api/san-pham?ma_loai_san_pham=LUvHhrXge1xlJDtTmsBd', {withCredentials: true}
                ).then(resp=>{
                  resp.data.data.map(
                    (item,index) => setOther(index +1)
                  )
                })
                axios.get('https://deploy-server-123.herokuapp.com/api/nguoi-dung/quan-ly', {withCredentials: true}
                ).then(resp=>{
                  resp.data.data.map(
